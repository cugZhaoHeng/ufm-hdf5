...
def animate_multiple_ufm_growth_sequentially(
    ...
    with h5py.File(file_path, "r") as f:
        ...
        for s_id in sim_ids:
            try:
                ...
                logger.info(f"  - 成功加载 sim {s_id}: 共 {data['n_time']} 个时间步")
            except Exception as e:
                logger.info(f"  - 跳过 sim {s_id}，错误原因: {e}")
    ...
    if remove_origin:
        ...
        for s_data in sim_data_list:
            ...
            for t in range(s_data["n_time"]):
                # ElementCount 的值是变化的，不能直接用 n_element_max 来切片，否则会包含大量无效数据
    ...
    logger.info("=" * 60)
    logger.info(f"顺序展示裂缝数量: {len(sim_data_list)}")
    logger.info(f"统一色标范围: [{vmin:.4f}, {vmax:.4f}] {value_unit}")
    logger.info("=" * 60)

    # 4. 计算帧数映射
    # 每个模拟对应的帧数
    sim_frame_counts = [s_data["n_time"] for s_data in sim_data_list]
    cum_frames = np.cumsum([0] + sim_frame_counts)  # 累计帧数区间
    ...
    def update(frame_idx):
        # 1. 寻找当前帧落在哪一个裂缝的生长区间
        # 满足 cum_frames[k] <= frame_idx < cum_frames[k+1]
        active_sim_idx = 0
        ...
        local_t = frame_idx - cum_frames[active_sim_idx]
        active_sim_id = sim_data_list[active_sim_idx]["sim_id"]

        all_verts = []
        all_colors = []
        summary_info = []

        # 2. 遍历所有预加载的裂缝，根据状态组装显示数据
        for idx, s_data in enumerate(sim_data_list):
            s_id = s_data["sim_id"]
            if idx < active_sim_idx:
                # 先前已完成生长的裂缝：保持在其最终帧
                t_show = s_data["n_time"] - 1
                status = "Finished (Static)"
            elif idx == active_sim_idx:
                # 当前正在生长的裂缝：处于生长中的局部帧
                t_show = local_t
                status = f"Growing ({local_t}/{s_data['n_time'] - 1})"
            else:
                # 后续裂缝：不渲染
                continue

            v, c = build_verts_for_sim_at_t(s_data, t_show)
            all_verts.extend(v)
            all_colors.extend(c)
            summary_info.append(f"Sim {s_id}: {status}, Patches={len(v)}")
        ...
        # 4. 更新文本信息框
        text_content = (
            f"Global Frame: {frame_idx + 1}/{total_frames}\n"
            f"Active Sim: {active_sim_id}\n"
    ...
    if save_path:
        ...
        logger.info(f"动画已成功保存至：{save_path}")
