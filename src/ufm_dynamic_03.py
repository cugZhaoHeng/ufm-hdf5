...
def animate_multiple_ufm_growth_sequentially(
    ...
    all_times = [s_data["n_time"] for s_data in sim_data_list]

    if len(set(all_times)) != 1:
        raise ValueError(
            f"检测到时间步不一致: {all_times}"
        )

    total_frames = all_times[0]
    ...
    def update(frame_idx):
        ...
        info_text.set_text(
            f"Time Step: {frame_idx+1}/{total_frames}\n"
