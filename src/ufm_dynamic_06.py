...
well_data_path = PROJECT_DIR / "data" / "well_data" / "68-4HF-02.txt"
...
def load_well_trajectory(file_path):
    """
    读取真实的测斜数据，保留可能使用的所有基准面 Z 坐标候选列。
    """
    try:
        # 自动跳过 Petrel 头的注释，读取 10 列测斜参数
        df = pd.read_csv(file_path, sep=r'\s+', comment='#')
        return {
            "X": df['X'].values,
            "Y": df['Y'].values,
            "ELEV": df['Z'].values,       # 候选 1: 原始海拔 Z（负值，最大到 -1777m）
            "SSTVD": -df['Z'].values,     # 候选 2: SSTVD 海拔深度（正值，最大到 +1777m）
            "TVD": df['TVD'].values,      # 候选 3: TVD 垂深（绝对正值，最大到 2416m）
        }
...
def animate_fractures_and_well_simultaneously(
    ...
    # 3. 【核心物理对齐】智能比对并选择与 H5 裂缝最贴合的井轨迹 Z 坐标体系
    ...
    diffs = {
        "TVD (垂深, 约 2400m)": abs(mean_frac_z - mean_well_tvd),
        "SSTVD (海拔深, 约 1700m)": abs(mean_frac_z - mean_well_sstvd),
        "ELEV (原始海拔, 约 -1700m)": abs(mean_frac_z - mean_well_elev)
    }
    best_fit = min(diffs, key=diffs.get)

    print("=" * 60)
    print(f"H5 裂缝坐标 Z 轴平均深度: {mean_frac_z:.2f} m")
    print(f"井轨迹均值 -> TVD: {mean_well_tvd:.2f} m | SSTVD: {mean_well_sstvd:.2f} m | Elevation: {mean_well_elev:.2f} m")
    print(f"--> [智能识别成功]：H5 文件采用 {best_fit} 坐标体系。")
    print("    系统已将井的 Z 坐标调整为此基准，确保井眼精确穿过裂缝面！")
    print("=" * 60)

    if best_fit == "TVD (垂深, 约 2400m)":
        well_data["Z_aligned"] = well_data["TVD"]
    elif best_fit == "SSTVD (海拔深, 约 1700m)":
        well_data["Z_aligned"] = well_data["SSTVD"]
    else:
        well_data["Z_aligned"] = well_data["ELEV"]

    # 4. 确定统一的全局基准原点 (X0, Y0, Z0)
    if remove_origin:
        all_x = list(well_data["X"])
        all_y = list(well_data["Y"])
        all_z = list(well_data["Z_aligned"])
    ...
    well_z_plot = well_data["Z_aligned"] - Z0
    ...
    # 7. 静态绘制已校准对齐的实测井轨迹（粗黑线）与井口标记（绿三角）
...
if __name__ == "__main__":
    ...
    animate_fractures_and_well_simultaneously(
        ...
        cmap_name="jet",
        alpha=0.9,               # 设置轻微透明，方便透视穿过其中的井轨迹
        edge_width=0.01,
        save_path=str(combined_save_path), # 若不需要后台写盘，可设为 None
    )